...
TEST(UsbCanClientTest, simple_test) {
  ...
  int32_t num = 0;
  EXPECT_EQ(usb_can_client.Send(frames, &num), ErrorCode::OK);
  EXPECT_EQ(usb_can_client.Receive(&frames, &num), ErrorCode::OK);
  CanFrame can_frame;
  frames.push_back(can_frame);
  EXPECT_EQ(usb_can_client.SendSingleFrame(frames), ErrorCode::OK);
  usb_can_client.Stop();
}
