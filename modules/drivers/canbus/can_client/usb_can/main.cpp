...
void *receive_func(void *param)  //接收线程。
{
  ...
  while ((*run) & 0x0f) {
    if ((reclen = VCI_Receive(VCI_USBCAN2, 0, ind, rec, 3000, 100)) >
        0)  //调用接收函数，如果有数据，进行数据处理显示。
    {
      for (j = 0; j < reclen; j++) {
        ...
        if (rec[j].ID != 0x08) {
          printf(" velicity:%04f", 
                 (rec[j].Data[2] * 256 + rec[j].Data[3]) * 0.1);
          printf(" x_position:%04f",
                 (rec[j].Data[4] * 256 + rec[j].Data[5]) * 0.1);
          printf(" y_position:%04f",
                 (rec[j].Data[6] * 256 + rec[j].Data[7]) * 0.1);
        }
...
int main() {
  ...
  if (VCI_ReadBoardInfo(VCI_USBCAN2, 0, &pInfo) ==
      1)  //读取设备序列号、版本等信息。
  {
    printf(">>Get VCI_ReadBoardInfo success!\n");
  ...
  // 	send[0].Data[i] = i;
  // }
  send[0].Data[0] = send[0].Data[1] = send[0].Data[3] = send[0].Data[2] =
      send[0].Data[4] = send[0].Data[5] = 0X00;
  send[0].Data[6] = 0XCA;
  send[0].Data[6] = 0XFF;
  ...
  usleep(10000000);  //延时单位us，这里设置 10 000 000=10s
                     //10s后关闭接收线程，并退出主程序。
  m_run0 = 0;        //线程关闭指令。
  pthread_join(threadid, NULL);     //等待线程关闭。
  usleep(50000);                   //延时100ms。
