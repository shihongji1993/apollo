...
#include "modules/drivers/radar/cub_radar/protocol/object_general_info_60b.h"
...
void ObjectGeneralInfo60B::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  auto cub_obs = cub_radar->add_contiobs();
  cub_obs->set_clusterortrack(false);
  cub_obs->set_obstacle_id(obj_id);
  cub_obs->set_longitude_dist(longitude_dist(bytes, length));
  cub_obs->set_lateral_dist(lateral_dist(bytes, length));
  cub_obs->set_longitude_vel(longitude_vel(bytes, length));
  cub_obs->set_lateral_vel(lateral_vel(bytes, length));
  cub_obs->set_rcs(rcs(bytes, length));
  cub_obs->set_dynprop(dynprop(bytes, length));
  double timestamp = apollo::common::time::Clock::NowInSeconds();
  auto header = cub_obs->mutable_header();
