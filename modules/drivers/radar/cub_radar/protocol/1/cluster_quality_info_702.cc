...
#include "modules/drivers/radar/cub_radar/protocol/cluster_quality_info_702.h"
...
void ClusterQualityInfo702::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  for (int i = 0; i < cub_radar->contiobs_size(); ++i) {
    if (cub_radar->contiobs(i).obstacle_id() == id) {
      auto cub_obs = cub_radar->mutable_contiobs(i);
      cub_obs->set_longitude_dist_rms(
          LINEAR_RMS[longitude_dist_rms(bytes, length)]);
      cub_obs->set_lateral_dist_rms(
          LINEAR_RMS[lateral_dist_rms(bytes, length)]);
      cub_obs->set_longitude_vel_rms(
          LINEAR_RMS[longitude_vel_rms(bytes, length)]);
      cub_obs->set_lateral_vel_rms(LINEAR_RMS[lateral_vel_rms(bytes, length)]);
      cub_obs->set_probexist(PROBOFEXIST[pdh0(bytes, length)]);
      switch (invalid_state(bytes, length)) {
        ...
        case 0x07:
        case 0x0E:
          cub_obs->set_probexist(PROBOFEXIST[0]);
        default:
          break;
      }
