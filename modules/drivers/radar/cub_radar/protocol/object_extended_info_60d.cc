...
#include "modules/drivers/radar/cub_radar/protocol/object_extended_info_60d.h"
...
void ObjectExtendedInfo60D::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  for (int i = 0; i < cub_radar->contiobs_size(); ++i) {
    if (cub_radar->contiobs(i).obstacle_id() == obj_id) {
      auto obs = cub_radar->mutable_contiobs(i);
  ...
  // auto cub_obs = cub_radar->mutable_contiobs(object_id(bytes, length));
}
